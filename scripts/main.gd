...
func _ready() -> void:
	global.load_game()
	if global.final_desbloqueado == true:
		get_tree().change_scene_to_file.call_deferred("res://tscn's/fin_juego.tscn")
	else:
		pass
...
func _on_barrier_world_area_entered(area: Area2D) -> void:
	if area.name == "attack_colider":
		...
		if vida_barrera == 0:
			$"personaje principal/music_and_sounds/pimin_cave".stop()
			$"personaje principal/music_and_sounds/strange_sky".stop()
			$f.queue_free()
...
func _on_final_juego_1_body_entered(body: Node2D) -> void:
	if body.name == "personaje principal":
		await global.cambiar_final_juego()
